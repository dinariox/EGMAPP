...
<template>
  <f7-page>
        ...
        <f7-navbar v-if="$theme.ios">
            ...
            <f7-nav-center sliding>Admin-Bereich: News</f7-nav-center>
        </f7-navbar>
        ...
        <div class="text" v-if="hasPermission">
            ...
            <h2 class="popup-title" v-if="!editMode">Artikel aus dem "News"-Bereich</h2>
            <h2 class="popup-title" v-if="editMode">Artikel bearbeiten</h2>

            <p v-if="!editMode && !newMode">
                Hier gibt es eine Übersicht mit allen Artikeln die im Bereich "News" hochgeladen wurden.
...
export default {
  ...
  methods: {
    ...
    updateImageUrl: function (newImgUrl) {
      ...
      this.$root.db('news/' + this.idTB).once('value').then(function (snapshot) {
    ...
    uploadNewImgUrl: function (newImgUrl, oldInfo) {
      this.$root.db('news/' + this.idTB).set({
        image: newImgUrl,
        titel: oldInfo.titel,
        text: oldInfo.text,
        icon: oldInfo.icon,
    ...
    uploadEditedArticle: function (image) {
      ...
      this.$root.db('news/' + this.idTB).set(updateInfo).then(() => {
        window.f7.hideIndicator()
        ...
        window.f7.alert('Der Artikel wurde erfolgreich veröffentlicht!', '⚙️ Erfolg', () => {
          ...
          me.$root.db('log/' + new Date().getTime()).set({
            ...
            action: 'Updated News ' + updateInfo.link + ': [TITEL] ' + updateInfo.titel + ' [TEXT] ' + updateInfo.text + ' [IMAGE] ' + updateInfo.image
          })
        })
      }).catch((error) => {
    ...
    uploadNewArticle: function (newId) {
      ...
      this.$root.db('news/' + newId).set(updateInfo).then(() => {
        window.f7.hideIndicator()
        ...
        window.f7.alert('Der Artikel wurde erfolgreich erstellt!', '⚙️ Erfolg', () => {
          ...
          me.$root.db('log/' + new Date().getTime()).set({
            user: me.$root.user.uid,
            action: 'Uploaded News ' + newId + ': [TITEL] ' + updateInfo.titel + ' [TEXT] ' + updateInfo.text + ' [IMAGE] ' + updateInfo.image
          })
        })
      }).catch((error) => {
        window.f7.alert('Sorry, etwas ist schiefgelaufen!<br /><br />' + error.message, '⚙️ Fehler')
